...
const LoginModal = ({ visible, onClose}: Props) => {
  ...
  const loginMutation = useMutation({
    ...
    mutationFn: (data: Authentication) => {
        return axios.post("http://localhost:5000/api/users/login", data, { withCredentials: true }).then(response => response.data);
    },
    onSuccess: (data) => {
        setJWT(data?.token);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-25 backdrop-blur-sm flex justify-center items-center">
        <div className="w-2/5">
            <div className="bg-white p-4 flex flex-col rounded-lg">
                <ModalHeader title="Login" description="Access your existing account and start predicting stock prices!" onClose={onClose} />
